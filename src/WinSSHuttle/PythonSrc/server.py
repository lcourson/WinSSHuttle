...
class DnsProxy(Handler):
    ...
    def try_send(self):
        ...
        debug2('     S: DNS: sending to %r:%d (try %d)' % (peer, port, self.tries))
        try:
            ...
            if e.args[0] in ssnet.NET_ERRS:
                ...
                debug2('     S: DNS send to %r: %s' % (peer, e))
                self.try_send()
                return
            else:
                log('     S: DNS send to %r: %s' % (peer, e))
                return
    ...
    def callback(self, sock):
        ...
        try:
            ...
            if e.args[0] in ssnet.NET_ERRS:
                ...
                # Note: these errors sometimes are reported by recv(),
                # and sometimes by send().  We have to catch both.
                debug2('     S: DNS recv from %r: %s' % (peer, e))
                self.try_send()
                return
            else:
                log('     S: DNS recv from %r: %s' % (peer, e))
                return
        debug2('     S: DNS response: %d bytes' % len(data))
...
class UdpProxy(Handler):
    ...
    def send(self, dstip, data):
        debug2('     S: UDP: sending to %r port %d' % dstip)
        try:
            self.sock.sendto(data, dstip)
        except socket.error:
            _, e = sys.exc_info()[:2]
            log('     S: UDP send to %r port %d: %s' % (dstip[0], dstip[1], e))
            return
    ...
    def callback(self, sock):
        try:
            ...
            _, e = sys.exc_info()[:2]
            log('     S: UDP recv from %r port %d: %s' % (peer[0], peer[1], e))
            return
        debug2('     S: UDP response: %d bytes from %s,%r' % (len(data), peer[0], peer[1]))
...
def main(latency_control, latency_buffer_size, auto_hosts, to_nameserver,
    ...
    try:
        ...
        debug1('     S: latency control setting = %r' % latency_control)
        ...
        debug1('     S: auto-nets:' + str(auto_nets))
        if auto_nets:
            routes = list(list_routes())
            debug1('     S:   available routes:')
            for r in routes:
                debug1('     S:   %d/%s/%d' % r)
        else:
            routes = []
        ...
        def dns_req(channel, data):
            debug2('c -> S: Incoming DNS request channel=%d.' % channel)
        ...
        def udp_req(channel, cmd, data):
            debug2('c -> S: Incoming UDP request channel=%d, cmd=%d' %
                   (channel, cmd))
            if cmd == ssnet.CMD_UDP_DATA:
                (dstip, dstport, data) = data.split(b(','), 2)
                dstport = int(dstport)
                debug2('     S:   is incoming UDP data. %r %d.' % (dstip, dstport))
                h = udphandlers[channel]
                h.send((dstip, dstport), data)
            elif cmd == ssnet.CMD_UDP_CLOSE:
                debug2('     S:   is incoming UDP close')
                h = udphandlers[channel]
                h.ok = False
                del mux.channels[channel]

        def udp_open(channel, data):
            debug2('c -> S: Incoming UDP open.')
            family = int(data)
            #debug2('     S: Family: %d' % family)
        ...
        while mux.ok:
            ...
            if dnshandlers:
                ...
                for channel, h in dnshandlers.items():
                    if h.timeout < now or not h.ok:
                        debug3('     S: expiring dnsreqs channel=%d' % channel)
            ...
            if udphandlers:
                ...
                for channel, h in udphandlers.items():
                    if not h.ok:
                        debug3('     S: expiring UDP channel=%d' % channel)
                        remove.append(channel)
                        h.ok = False
                for channel in remove:

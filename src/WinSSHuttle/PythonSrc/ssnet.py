...
def _nb_clean(func, *args):
    try:
        ...
        if e.errno not in (errno.EWOULDBLOCK, errno.EAGAIN):
            ...
            debug3('     S: %s: err was: %s' % (func.__name__, e))
            return None
...
class SockWrapper:
    ...
    def __init__(self, rsock, wsock, connect_to=None, peername=None):
        global _swcount
        _swcount += 1
        debug3('     S: creating new SockWrapper (%d now exist)' % _swcount)
        self.exc = None
        self.rsock = rsock
        self.wsock = wsock
    ...
    def __del__(self):
        global _swcount
        _swcount -= 1
        debug1('     S: %r: deleting (%d remain)' % (self, _swcount))
        if self.exc:
            debug1('     S: %r: error was: %s' % (self, self.exc))
    ...
    def try_connect(self):
        ...
        debug3('     S: %r: trying connect to %r' % (self, self.connect_to))
        try:
            ...
            _, e = sys.exc_info()[:2]
            debug3('     S: %r: connect result: %s' % (self, e))
            if e.args[0] == errno.EINVAL:
                ...
                debug3('     S: %r: fixed connect result: %s' % (self, e))
    ...
    def noread(self):
        if not self.shut_read:
            debug2('     S: %r: done reading' % self)
            self.shut_read = True
            # self.rsock.shutdown(SHUT_RD)  # doesn't do anything anyway

    def nowrite(self):
        if not self.shut_write:
            debug2('     S: %r: done writing' % self)
    ...
    def uwrite(self, buf):
        ...
        try:
            ...
            if e.errno == errno.EPIPE:
                debug1('     S: %r: uwrite: got EPIPE' % self)
                self.nowrite()
                return 0
            else:
...
class Handler:
    ...
    def callback(self, sock):
        log('     S: --no callback defined-- %r' % self)
...
class Mux(Handler):
    ...
    def send(self, channel, cmd, data):
        ...
        #head = struct.pack('!ccHHH', b('S'), b('S'), channel, cmd, len(data))
        #debug2('c <- S:%d Header: %r' % (channel, ":".join("{:02x}".format(c) for c in head)))
        #debug2('c <- S:%d Channel: %d' % (channel, channel))
        #debug2('c <- S:%d CMD: %s' % (channel, cmd_to_name.get(cmd, hex(cmd))))
        #debug2('c <- S:%d len: %d' % (channel, len(data)))
        ##self.log_data('c <- S:%d Data: %r' % (channel, ":".join("{:02x}".format(c) for c in data)))
        #debug2('c <- S:%d Data: %r' % (channel, ":".join("{:02x}".format(c) for c in data)))
        p = struct.pack('!ccHHH', b('S'), b('S'), channel, cmd, len(data)) \
            + data
        #self.log_data('c <- S: %d:Data2: %r' % (channel, ":".join("%r" % c for c in p)))
        self.outbuf.append(p)
        debug2('c <- S: channel=%d cmd=%s len=%d (fullness=%d)'
               % (channel, cmd_to_name.get(cmd, hex(cmd)),
                  len(data), self.fullness))
        self.fullness += len(data)

    def got_packet(self, channel, cmd, data):
        debug2('c -> S: channel=%d cmd=%s len=%d'
        ...
        if cmd == CMD_PING:
            ...
            #    pingBytes[i] = b'%r' % i
            #
            #self.send(0, CMD_PONG, pingBytes)
            self.send(0, CMD_PONG, data)
        elif cmd == CMD_PONG:
            debug2('     S:%d received PING response' % channel)
            self.too_full = False
            self.fullness = 0
        elif cmd == CMD_EXIT:
            self.ok = False
            ...
            if not callback:
                log('     S: warning: closed channel %d got cmd=%s len=%d'
                    % (channel, cmd_to_name.get(cmd, hex(cmd)), len(data)))
            else:
                callback(cmd, data)
    ...
    def flush(self):
        ...
        if self.outbuf and self.outbuf[0]:
            ...
            debug2('     S: mux wrote: %r/%d' % (wrote, len(self.outbuf[0])))
            if wrote:
    ...
    def handle(self):
        ...
        while 1:
            if len(self.inbuf) >= (self.want or HDR_LEN):
                ...
                #debug2('c -> S: %d:Header: %r' % (channel, ":".join("{:02x}".format(c) for c in self.inbuf[:HDR_LEN])))
                #debug2('c -> S: %d:Channel: %d' % (channel, channel))
                #self.log_data('c -> S: %d:CMD: %s' % (channel, cmd_to_name.get(cmd, hex(cmd))))
                #self.log_data('c -> S: %d:len: %d' % (channel, datalen))
                assert(s1 == b('S'))
                assert(s2 == b('S'))
                self.want = datalen + HDR_LEN
            if self.want and len(self.inbuf) >= self.want:
                data = self.inbuf[HDR_LEN:self.want]
                #self.log_data('c -> S: %d:Data: %r' % (channel, data))
                #debug2('c -> S: %d:Data2 %r' % (channel, ":".join("{:02x}".format(c) for c in data)))
...
class MuxWrapper(SockWrapper):
    ...
    def __init__(self, mux, channel):
        ...
        self.socks = []
        debug2('     S: new channel: %d' % channel)
    ...
    def maybe_close(self):
        if self.shut_read and self.shut_write:
            debug2('     S: %r: closing connection' % self)
...
def connect_dst(family, ip, port):
    debug2('     S: Connecting to %s:%d' % (ip, port))
...
def runonce(handlers, mux):
    ...
    for s in handlers:
        s.pre_select(r, w, x)
    debug2('     S: Waiting: %d r=%r w=%r x=%r (fullness=%d/%d)'
           % (len(handlers), _fds(r), _fds(w), _fds(x),
               mux.fullness, mux.too_full))
    (r, w, x) = select.select(r, w, x)
    debug2('     S:   Ready: %d r=%r w=%r x=%r'
           % (len(handlers), _fds(r), _fds(w), _fds(x)))
    ready = r + w + x
    did = {}
    for h in handlers:
